...
function buscar(div_text) {
	...
	for (let index = 0; index < aCartes.length; index++) {
		...
		//Para que no se te colen en la busqueda pokemon que no empiecen por el texto buscado
		// existe una función startsWith (https://developer.mozilla.org/es/docs/Web/JavaScript/Reference/Global_Objects/String/startsWith)
		// que te devuele true si el texto empieza con el elemento buscado
		// ahora mismo se estan colando pokemon que no empiezan con el texto buscado ejemplo: si pones la letra c en el buscador
		// aparecen todos los pokemon que empiezan por c y también sirfetchd porque contiene la letra c
		// nom.startsWith(div_text.value) solo te devolverá aquellos pokemon que empiecen por el texto buscado.
...
function flip_card_func(obj) {
	...
	if (flipped_cards_index == 2) {
		...
		setTimeout(function () {
			// Executar funció després d'un retard de 0.5s per deixar temps perquè es gira la segona carta.
			// Missatge resultat del combat:
			if (Number(flipped_cards_array[1].card_atac) > Number(flipped_cards_array[2].card_defensa)) {
				alert(
					"'" +
						flipped_cards_array[1].card_nom +
						"' ataca i guanya a '" +
						flipped_cards_array[2].card_nom +
						"'"
				)
			} else {
				alert(
